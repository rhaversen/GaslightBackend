...
import { type DiePair, type StrategyAPI } from './types.js'
...
export function createStrategyAPI (playerIndex: number): StrategyAPI {
	...
	return {
		...
		detEllerDerover: () => {
			...
			const previousAnnouncedValue = gameState.getPreviousActions()[0]?.announcedValue ?? score
		...
		reveal: () => {
			...
			if (previousPlayerValue !== undefined && previousPlayerAnnouncedValue !== undefined && previousActionType === 'roll') {
				previousPlayerLied = previousPlayerValue !== previousPlayerAnnouncedValue
			} else if (previousPlayerValue !== undefined && previousPlayerAnnouncedValue !== undefined && previousActionType === 'detEllerDerover') {
				previousPlayerLied = previousPlayerValue < previousPlayerAnnouncedValue
			}
		...
		lie: (diePair: DiePair) => {
			...
			if (realValue === undefined) {
				throw new Error('No rolled action to lie about.')
			}

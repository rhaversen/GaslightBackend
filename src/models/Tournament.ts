import { type Document, model, Schema, type SortOrder, Types } from 'mongoose'

import GradingModel from './Grading.js'
import { type IGrading, type IGradingPopulated } from './Grading.js'
import SubmissionModel from './Submission.js'
import { type ISubmissionPopulated } from './Submission.js'
...
tournamentSchema.methods.getStandings = async function (limit: number = 0, skip: number = 0, sortField: string = 'score', sortOrder: SortOrder = -1) {
	...
	const submissions = await SubmissionModel
		.find({ _id: { $in: gradings.map(g => g.submission.toString()) } })
		.populate('user', 'username')
		.exec() as unknown as ISubmissionPopulated[]
...
tournamentSchema.methods.calculateStatistics = async function () {
	...
	const extrema = {
		minimum: scores[0] ?? 0, // array is already sorted
...
tournamentSchema.path('gradings').validate(async function (v: Types.ObjectId[]) {
	...
	const gradings = await GradingModel.find({ _id: { $in: v } }).populate({
		path: 'submission',
		populate: { path: 'user' }
	}).exec() as unknown as IGradingPopulated[]
	const submissions = gradings.map(grading => grading.submission) as unknown as ISubmissionPopulated[]
...
tournamentSchema.path('gradings').validate(async function (v: Types.ObjectId[]) {
	const gradings = await GradingModel.find({ _id: { $in: v } })
		...
		.exec() as unknown as { submission: { game: Schema.Types.ObjectId } }[]
